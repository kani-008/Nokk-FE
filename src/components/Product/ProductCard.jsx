...
export default function ProductCard({ product, selectedWeights = [], itemType = "product", combo }) {
  ...
  const wishlisted = !isCombo ? isWishlisted(product.id) : false;
  ...
  const offer = !isCombo ? product.activeOffer : null;
  const offerPrice = offer ? applyActiveOffer(rawPrice, offer) : null;

  const price = isCombo ? combo.comboPrice : (offerPrice != null ? offerPrice : rawPrice);
  const compare = isCombo ? combo.individualTotal : (offerPrice != null ? rawPrice : rawCompare);
  const hasDisc = isCombo ? (combo.individualTotal > combo.comboPrice) : (compare > price);
  const disc = isCombo ? Math.round(((combo.individualTotal - combo.comboPrice) / combo.individualTotal) * 100) : (hasDisc ? Math.round(((compare - price) / compare) * 100) : 0);
  ...
  if (isCombo && combo.items) {
    try {
      const totalWeight = combo.items.reduce((sum, it) => {
        const match = it.weightLabel.match(/(\d+)/);
        return sum + (match ? parseInt(match[1]) * it.quantity : 0);
      }, 0);
      if (totalWeight > 0) {
        cardWeightLabel = `${totalWeight}g`;
      } else {
        cardWeightLabel = "Combo Pack";
      }
    } catch {
      cardWeightLabel = "Combo Pack";
    }
  ...
  const cartItem = isCombo
    ? items.find((i) => i.comboId === combo.id)
    : (firstV ? items.find((i) => i.variantId === firstV.id) : null);
      ...
      <div>
        ...
        <div className="relative aspect-square overflow-hidden bg-gray-100 rounded-t-md">
          {image ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center bg-sandal-50 text-sandal-400">
              ...
              <span className="text-[10px] uppercase font-bold tracking-wider font-body">No Image</span>
            </div>
          )}

          {/* Best Seller / New / Offer labels */}
          <div className="absolute top-2.5 left-2.5 flex flex-col items-start gap-1">
            {!isCombo && product.isBestseller && (
        ...
        <div className="p-4.5">
          <p className="font-body text-[10px] text-sandal-600 uppercase tracking-widest font-extrabold mb-1">
            {isCombo ? "Dry Fish" : product.categoryName}
          </p>
          ...
          {isCombo ? (
            <p className="font-body text-xs text-sandal-500 font-semibold mb-2 line-clamp-1 leading-normal">
              {combo.items ? combo.items.map(it => `${it.quantity} × ${it.productName}`).join(" + ") : ""}
            </p>
          ) : (
            product.nameTa && (
