    ...
    try {
      ...
      setError(
        e.response?.data?.message ||
          e.message ||
          "Could not submit replacement request",
      );
    }
  };
  ...
  return (
    <Popup title="Request Replacement" onClose={onClose}>
      <div className="space-y-4">
        <div>
          <label className="font-body text-xs font-semibold text-amber-700 mb-1.5 block">
            Reason
          </label>
      ...
      <div className="space-y-3 lg:space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-display text-lg lg:text-xl font-bold text-brand-900">
              Order Details
            </h2>
          </div>
          <a
            href="https://wa.me/918870485722?text=Hello,%20I%20need%20help%20with%20my%20order."
            target="_blank"
            rel="noreferrer"
        ...
        >
          <div className="space-y-4">
            {displayItems.map((item, i) => (
              <div
                key={i}
                className="flex gap-3 items-start border-b border-amber-100/30 pb-3 last:border-none last:pb-0"
              >
                {item.imageUrl ? (
                  <img
                    src={item.imageUrl}
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-body text-sm font-bold text-brand-900 line-clamp-1">
                    {item.productName}
                  </p>
        ...
        {hasTracking && (
          <div>
            <p className="font-body text-xs font-semibold text-amber-800 uppercase tracking-wider mb-2">
              Track Status
            </p>
            <TrackingStatusCard
              order={order}
        ...
        {isDelivered && unreviewedDisplayItems.length > 0 && (
          <div className="space-y-3">
            <h3 className="font-display text-base lg:text-lg font-bold text-brand-900">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 lg:gap-4">
          <Section label="Delivery Address">
            <div>
              ...
              {order.customerPhone && (
                <p className="font-body text-xs text-amber-700 font-semibold mt-0.5">
                  {order.customerPhone}
                </p>
              )}
              <p className="font-body text-xs text-amber-600 mt-2 leading-relaxed">
                ...
                {addr.taluk?.trim() && addr.taluk.trim().toUpperCase() !== "NA"
                  ? `${addr.taluk.trim()}, `
                  : ""}
          ...
          <div className="pt-1">
            {canRequestReplacement && (
              <button
                ...
                title={
                  !isWithin24Hours
                    ? "Replacement requests are only allowed within 24 hours of delivery"
                    : ""
                }
              >
                <RotateCcw size={13} /> Request Replacement
                {!isWithin24Hours && " (Expired)"}
              </button>
            )}
            {replacementRequested && (
              <div className="bg-amber-50 rounded-xl p-3 border border-amber-100 flex items-center justify-center text-xs text-amber-800 font-semibold gap-1.5">
                <Check size={14} className="text-green-600" /> Replacement
                requested
              </div>
            )}
          </div>
