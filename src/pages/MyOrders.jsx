...
// ── Order card ─────────────────────────────────────────────────────────
// ── Order card ─────────────────────────────────────────────────────────
function OrderCard({ order, onClick, onWriteReviewClick }) {
  ...
  return (
    <div
      onClick={onClick}
      className="card flex flex-col px-4 sm:px-5 py-4 cursor-pointer hover:bg-amber-50/50 transition-colors"
    >
      <div className="flex items-center justify-between w-full">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            {/* Mobile view layout (matching the reference screenshot) */}
            <div className="block sm:hidden">
              <h3 className="font-body text-xs font-bold text-gray-900 leading-snug">
                {statusTitle}
              </h3>
              <p className="font-body text-[11px] text-gray-500 mt-1 line-clamp-1">
                {subtitleText}
              </p>
            </div>

            {/* Desktop view layout (preserved format) */}
            <div className="hidden sm:block">
              <h3 className="font-body text-sm font-semibold text-brand-900 leading-snug line-clamp-1">
                {itemName}
              </h3>
              {hasMore && (
...
export default function MyOrders() {
  ...
  if (selectedOrder) {
    return (
      <OrderDetail
        ...
        onStatusUpdate={(newStatus) => setSelectedOrder((prev) => ({ ...prev, status: newStatus }))}
      />
    );
  }
  ...
  return (
    <div className="page-wrap py-2">
      <h2 className="font-display text-2xl font-bold text-brand-900 mb-4">My Orders</h2>
